...
description: SONiC の BGP 詳細情報（AS path / in-out RIB 等）を可視化する BMP 実装。openbmpd を bmp コンテナに同梱し FRR bgpd → BMP（RFC 7854）→ openbmpd → BMP_STATE_DB 経路で保存、gNMI streaming で公開する。
area: routing
