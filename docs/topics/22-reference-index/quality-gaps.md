...
機能章の実装は、機能章ごとに 1 PR で進める。各章実装後に reference 側へ波及する変更が出た場合は、別 PR (reference バッチ) で reference 辞書を追加する。両者を 1 PR に混ぜないことで、章のレビュー観点 (読み物としての構造) と reference のレビュー観点 (辞書の正確さ) を分離できる。
