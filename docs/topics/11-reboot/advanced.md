...
- **Express reboot**: warm reboot のさらに先にある「ASIC を温存したまま OS だけ swap する」設計。`fast-reboot-flow-improvements` と `express-reboot` HLD を組み合わせて、SAI 状態の export / import と syncd の handover を最小ダウンタイムで行う。
...
- Smart switch / DASH 構成 ([13](../13-dash-smartswitch/index.md)) では DPU 側の独立した reboot 経路が追加され、NPU と DPU の協調 reboot の設計が future work。

## 関連 RFC / 仕様書

- [RFC 4724](https://datatracker.ietf.org/doc/html/rfc4724) — BGP Graceful Restart
