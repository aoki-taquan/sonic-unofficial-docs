...
description: Reboot / Upgrade の発展トピック — warm / fast / cold reboot の基本パスを押さえた後は、収束時間を縮める express
  reboot、multi-ASIC / chassis 級の同期、in-service upgrade との組合せが論点になる。express reboot は現状 Cisco 8000 系
  ASIC （加えて marvell-teralynx）に限定される点が最重要の前提となる。
area: topics
verification: meta
...
- **Express reboot**: warm reboot のさらに先にある「ASIC を温存したまま OS だけ swap する」設計。`fast-reboot-flow-improvements` と `express-reboot` [HLD](../../reference/glossary.md#term-hld) を組み合わせて、[SAI](../../reference/glossary.md#term-sai) 状態の export / import と [syncd](../../reference/glossary.md#term-syncd) の handover を最小ダウンタイムで行う。HLD で「express-reboot は Cisco 8000 platform 限定」と明文化されており[^express-platform]、master の `fast-reboot` script でも `asic_type` が `cisco-8000` または `marvell-teralynx` 以外なら即時 abort する[^express-script]。これ以外の platform では本トピックは適用外と理解してよい。
...
Express reboot は warm reboot の延長で、SAI 状態と data plane を保持したまま control plane だけ swap する。鍵となるのは (1) `syncd` の SAI object handle dump と新 image の `syncd` での再 attach、(2) `orchagent` の internal cache を `APPL_STATE_DB` 経由でリビルド、(3) `bgpd` の GR/LLGR による経路再学習を上位 plane 側でブロックしない、という 3 つの同期点。HLD は「sub-second data plane interruption」を目標として明示しており[^express-goal]、warm reboot (数秒〜十数秒) より大幅に短いダウンタイムを狙う。

実装の依存は [ASIC SDK](../../reference/glossary.md#term-asic-sdk) 側の `SAI_SWITCH_ATTR_FAST_API_ENABLE` と、ベンダー SDK が持つ external phy 非初期化 / NPU 再 attach の機構である。HLD では `syncd_request_shutdown --pxe` で pre-shutdown を通知し、再起動後 `SAI_KEY_BOOT_TYPE=express` で syncd を立ち上げる流れが定義されている[^express-syncd]。`SAI_SWITCH_ATTR_FAST_API_ENABLE` は元々 `fastfast` boot で導入された属性を流用したものであり、新規 attribute ではない点に注意[^express-syncd]。これに対応していない platform では express reboot は不可。
...
- **Express reboot の platform 依存**: 上述のとおり master 時点では `cisco-8000` / `marvell-teralynx` のみ通過する gating があり[^express-script]、それ以外の ASIC では script レベルで即時失敗する。導入検討時はまず実機の `asic_type` を確認する。
...
[^express-platform]: SONiC/doc/express-reboot/Cisco_8000_Express_Reboot_HLD.md L26 「Enforce express-reboot is only applicable to Cisco 8000 platforms at the moment.」
[^express-script]: [sonic-utilities](../../reference/glossary.md#term-sonic-utilities)/scripts/fast-reboot L931-L936 `asic_type` が `cisco-8000` でも `marvell-teralynx` でもない場合に「eXpress Boot is not supported」を出力して非ゼロ終了する gating。
[^express-goal]: SONiC/doc/express-reboot/Cisco_8000_Express_Reboot_HLD.md L6 「The goal of Sonic express reboot is to be able to restart and upgrade SONiC software with sub-second data plane interruption.」
[^express-syncd]: SONiC/doc/express-reboot/Cisco_8000_Express_Reboot_HLD.md L27-L28, L37-L54, L106, L131-L135 — `syncd_request_shutdown -pxe` / `SAI_KEY_BOOT_TYPE=SAI_START_TYPE_EXPRESS_BOOT` / `SAI_SWITCH_ATTR_FAST_API_ENABLE` の流用についての記述。
