...
native YANG のソースは `sonic-buildimage` リポジトリの `src/sonic-yang-models/yang-models/` 配下にまとまっている <!-- evidence: sonic-net/sonic-buildimage src/sonic-yang-models/yang-models @9ea932ec (136 module) --> 。本ページに挙げる sonic-*.yang はすべてここに実在するモジュールに対応する。個別ページが未整備のモジュールは `.yang` のパス表記のままにしてある。
...
| 10 [gNMI](../../reference/glossary.md#term-gnmi) / OpenConfig | `sonic-gnmi.yang`, `sonic-telemetry.yang`, `sonic-telemetry_client.yang`, `sonic-grpcclient.yang`, `sonic-high-frequency-telemetry.yang` (個別ページは未整備、source: `src/sonic-yang-models/yang-models/`) <!-- evidence: sonic-net/sonic-buildimage src/sonic-yang-models/yang-models/sonic-{gnmi,telemetry,telemetry_client,grpcclient,high-frequency-telemetry}.yang @9ea932ec --> | gNMI / streaming telemetry / gRPC client 設定 |
...
| 18 P4 / [PINS](../../reference/glossary.md#term-pins) | （該当する `sonic-p4*.yang` / `sonic-pins*.yang` は master の yang-models 配下に存在しない <!-- evidence: sonic-net/sonic-buildimage src/sonic-yang-models/yang-models @9ea932ec — p4/pins 接頭辞のモジュール無し --> 。PINS 側スキーマは別リポ sonic-net/sonic-pins を参照） | P4 / PINS は YANG ではなく gNMI/P4Runtime で扱われる |
