...
VRF 指定時（`show ip bgp summary vrf Vrf_red` 等）は `vrf-id` が 0 ではなく VRF の数値 ID になり、neighbor 集合も VRF スコープに限定されます。
...
- `Up/Down` の `never` は一度も Established していない印。physical link / TCP / ACL / source IP / password を最初に疑います。
...
| FRR では RIB にあるが `show ip route` の `>` 印が無い | RIB-to-FIB 移行で失敗 | zebra ログ、fpmsyncd、APPL_DB:ROUTE_TABLE |
...
| CRM 残量 | `crm show resources all`、`crm show summary` |
...
- `APPL_DB:NEXTHOP_GROUP_TABLE:<id>` — ECMP nexthop group。member 数と weight。
- `STATE_DB:NEIGH_STATE_TABLE|<ip>` — `bgpmon` が書く CiscoBgp4MIB 用の neighbor state。
- `STATE_DB:BFD_SESSION_TABLE|...` — BGP に紐づく BFD session 状態。
...
- BGP service が起動しない: [運用入口](../01-overview/operations.md) の feature / hostcfgd 確認。
