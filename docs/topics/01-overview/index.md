...
- SONiC の設定は CLI、`config_db.json`、YANG、GCU、gNMI のどれを入口に読むべきか。
- `CONFIG_DB`、`APPL_DB`、`STATE_DB`、`ASIC_DB` はどの章の前提知識になるか。
- `config reload`、`config replace`、`config apply-patch`、rollback、ZTP、factory reset はどう使い分けるか。
...
- [設定データフロー](architecture.md): `CONFIG_DB` から daemon、`APPL_DB`、orchagent、SAI へ流れる全体像。
