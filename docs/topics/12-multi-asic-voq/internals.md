...
| `VoqOrch` / `SystemPortOrch` | `SystemNeighOrch`、`SystemLagOrch` 等 | system port を chassis 全体で広報、neighbor / LAG を chassis 規模で同期 |
...
- VOQ chassis では、各 ASIC の `CONFIG_DB` を**共通設定として一括反映**する仕組みが limited で、運用者は per-asic の config_db.json + chassis 全体 minigraph の二段管理になります。
