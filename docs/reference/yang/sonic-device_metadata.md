...
| `suppress-fib-pending` | `sonic-device_metadata/DEVICE_METADATA/localhost/suppress-fib-pending` | `enumeration` |  | disabled | enabled, disabled | Enable BGP suppress FIB pending feature. BGP will wait for route FIB installation before announcing routes |
| `async_swss_rec` | `sonic-device_metadata/DEVICE_METADATA/localhost/async_swss_rec` | `enumeration` |  | disabled | enabled, disabled | Enable asynchronous swss.rec recording in orchagent. |
