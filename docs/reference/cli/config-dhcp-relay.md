...
description: 'config dhcp_relay / dhcpv4_relay サブコマンド — config dhcp_relay および config dhcpv4_relay グループは docker-dhcp-relay コンテナの CLI プラグインとして実装され、インタフェースへの DHCP リレーアドレスの追加・削除を行う。'
area: reference
