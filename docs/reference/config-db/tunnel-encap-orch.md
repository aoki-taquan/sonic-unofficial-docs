...
| `SAI_TUNNEL_ATTR_ENCAP_TTL_VAL` | `encap_ttl` 引数の省略デフォルト `DEFAULT_TUNNEL_ENCAP_TTL = 255` (YANG 未定義フィールド)。ただし `encap_ttl == 0` のパスでは設定されない | `vxlanorch.h:49`, `vxlanorch.h:207`, `vxlanorch.cpp:392` |
...
| `VxlanTunnelMapOrch` での encap TTL | `createTunnelHw()` を `encap_ttl` 省略で呼ぶ → デフォルト引数 `DEFAULT_TUNNEL_ENCAP_TTL=255` が適用。`create_tunnel()` で `encap_ttl != 0` → `PIPE_MODEL + TTL=255` が SAI に設定される | `vxlanorch.cpp:2069`, `vxlanorch.h:207` |
...
```
src_creation_ == TNL_CREATION_SRC_EVPN  →  p2p = true
src_creation_ == TNL_CREATION_SRC_CLI   →  p2p = false (常に P2MP)
```
