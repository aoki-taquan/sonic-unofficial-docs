...
`DPU` テーブルは [SmartSwitch](../../reference/glossary.md#term-smartswitch) プラットフォームにおける物理 [DPU](../../reference/glossary.md#term-dpu) (Data Processing Unit) の設定情報を [CONFIG_DB](../../reference/glossary.md#term-config_db) に保持する[^1]。
エントリは `platform.json` から `sonic-config-engine/smartswitch_config.py` 経由で書き込まれ、minigraph 由来の IP アドレスとサービスポートを含む。

`orchagent` の `DashEniFwdOrch` が [DPU](../../reference/glossary.md#term-dpu) エントリを読み取り [ENI](../../reference/glossary.md#term-eni) フォワーディングを制御する。
`caclmgrd` は `swbus_port` を参照して iptables ルールを生成し、`sonic-gnmi` proxy は `gnmi_port` を用いて [DPU](../../reference/glossary.md#term-dpu) への [gNMI](../../reference/glossary.md#term-gnmi) 接続を確立する。
...
| `gnmi_port` | port-number | — (proxy fallback: `50052`) | DPU 上の [gNMI](../../reference/glossary.md#term-gnmi) サービス TCP ポート |
| `orchagent_zmq_port` | port-number | — (典型値: `5555`) | DPU [orchagent](../../reference/glossary.md#term-orchagent) の ZMQ サービス TCP ポート |
...
- `orchagent` (`DashEniFwdOrch`): `state` / `pa_ipv4` を必須フィールドとして読み取り、[ENI](../../reference/glossary.md#term-eni) フォワーディングルールを生成
- `caclmgrd` (`sonic-host-services`): `swbus_port` を読み取って iptables / ip6tables で DPU-to-DPU swbus 通信を許可; フィールド欠如時はその DPU config を無視
- `sonic-gnmi` DPU proxy (`dpuproxy/resolver.go`): `gnmi_port` を読み取り DPU [gNMI](../../reference/glossary.md#term-gnmi) 接続先を決定; 欠如時は `50052` にフォールバック
...
| フィールド | [YANG](../../reference/glossary.md#term-yang) default | コード由来デフォルト | 必須扱い | fallback 源 |
|-----------|-------------|-------------------|---------|------------|
| `state` | なし | なし | 実質必須 | `dashenifwdorch.h` `dpu_table_desc` required_attributes; 欠如時 request reject |
| `local_port` | なし | なし | 推奨 | プラットフォーム固有値; [orchagent](../../reference/glossary.md#term-orchagent) では直接参照なし |
...
| `orchagent_zmq_port` | なし | なし ([HLD](../../reference/glossary.md#term-hld) 典型値 `5555`) | 推奨 | フォールバック値なし; ZMQ 接続失敗で [orchagent](../../reference/glossary.md#term-orchagent) 機能停止の可能性 |
...
- **gnmi_port**: `sonic-gnmi` DPU proxy は DB 欠如時に `"50052"` を試み、さらに `["8080", "50052"]` の順で試行する (`resolver.go:103–110`)。[HLD](../../reference/glossary.md#term-hld) (`smart-switch-ha-detailed-design.md:337`) の記載典型値は `50051` であり、sample_config_db.json の実例値 `50052` と一致しない。実際の DPU サービスが listen するポートと一致させることが必要。

- **orchagent_zmq_port**: [HLD](../../reference/glossary.md#term-hld) 典型値 `5555`、sample_config_db.json では `50` (テスト値)。コード側に fallback なし。
...
| 2 | `DPU.state` + `DPU.pa_ipv4` の同時書き込み → orchagent [ENI](../../reference/glossary.md#term-eni) ルール生成 | **同時書き込み必須** | 欠如フィールドがあると orchagent が request reject |
...
| 5 | `DPU` テーブル ↔ CHASSIS_APP_DB | **連携なし** | [SmartSwitch](../../reference/glossary.md#term-smartswitch) と [VOQ](../../reference/glossary.md#term-voq) 構成は独立した DB セット |
...
**CHASSIS_APP_DB との非連携 (依存 #5)**: SmartSwitch DPU テーブルは [CONFIG_DB](../../reference/glossary.md#term-config_db) にのみ存在する。CHASSIS_APP_DB (`redis_chassis.server:6380`) は [VOQ](../../reference/glossary.md#term-voq) 構成のラインカード間 `SYSTEM_NEIGH` / `SYSTEM_LAG` 共有に使用されるが、SmartSwitch の `SmartSwitchModuleUpdater` はこれを使用しない（evidence: `chassisd:688-862`）。
...
| 8 | DPU が `CHASSIS_MIDPLANE_TABLE` ([STATE_DB](../../reference/glossary.md#term-state_db)) に不在 | `sonic-gnmi` DPU proxy | gNMI request が `NotFound` エラー | (gRPC ERROR) | `resolver.go:74-76` |
| 9 | [STATE_DB](../../reference/glossary.md#term-state_db) に `ip_address` フィールドなし | `sonic-gnmi` DPU proxy | gNMI request が `NotFound` エラー | (gRPC ERROR) | `resolver.go:80-83` |
...
| 定数名 | 値 | 説明 | 行 |
|--------|-----|------|-----|
| `StateDB` | `6` | [Redis](../../reference/glossary.md#term-redis) DB インデックス ([STATE_DB](../../reference/glossary.md#term-state_db)) | `resolver.go:10` |
| `ConfigDB` | `4` | [Redis](../../reference/glossary.md#term-redis) DB インデックス (CONFIG_DB) | `resolver.go:13` |
...
| 2 | `ENI` SET（初回）→ DPU 情報解決 | orchagent `EniFwdCtxBase` | [APPL_DB](../../reference/glossary.md#term-appl_db) `ACL_TABLE_TYPE\|ENI_REDIRECT` | [ACL](../../reference/glossary.md#term-acl) テーブルタイプを作成（matches: `dst_ip`, `inner_dst_mac`, `tunnel_term`） | `dashenifwdorch.cpp:603-625` |
| 3 | `ENI` SET（初回）→ DPU 情報解決 | orchagent `EniFwdCtxBase` | [APPL_DB](../../reference/glossary.md#term-appl_db) `ACL_TABLE\|ENI` | 外部物理/[LAG](../../reference/glossary.md#term-lag) ポートをバインドポートとして [ACL](../../reference/glossary.md#term-acl) テーブルを作成 | `dashenifwdorch.cpp:635-643` |
| 4 | `ENI` SET（ネクストホップ解決後）| orchagent `EniAclRule` | [APPL_DB](../../reference/glossary.md#term-appl_db) `ACL_RULE\|ENI:<vnet>:<mac>` | `redirect_action = DPU.pa_ipv4` の [ACL](../../reference/glossary.md#term-acl) ルールを set | `dashenifwdinfo.cpp:193-206` |
| 5 | `ENI` SET（LOCAL DPU 未解決時） | orchagent `LocalEniNH` | `NeighOrch` → [ARP](../../reference/glossary.md#term-arp) / [SAI](../../reference/glossary.md#term-sai) | DPU `pa_ipv4` に対する [ARP](../../reference/glossary.md#term-arp) 解決要求を発行。[ARP](../../reference/glossary.md#term-arp) 解決後コールバックで ACL ルール書き込み | `dashenifwdinfo.cpp:18-31` |
...
- **購読方式**: `swsscommon.Select` に登録し 1 秒タイムアウトのポーリングループで受信。内部は [Redis](../../reference/glossary.md#term-redis) keyspace 通知 (`__keyspace@<db>__:DPU|*` の psubscribe に相当)
...
| 差分ポイント | 具体的内容 | 証跡 |
|------------|-----------|------|
| SmartSwitch 判定 | `platform.json[DPUS]` 有無で `is_smartswitch()` が決まり、orchagent `DashEniFwdOrch` 起動のゲートになる | `device_info.py:671-680`, `orchdaemon.cpp:613` |
| DPU テーブル書き込み元 | SmartSwitch [NPU](../../reference/glossary.md#term-npu) 側: `smartswitch_config.py` が `platform.json[DPU]` を CONFIG_DB に転記。DPU ノード側: `platform.json[DPU]` は空 dict が典型で IP/ポートは minigraph 由来 | `smartswitch_config.py:43-46` |
| `dpu_reboot_timeout` | `platform.json` のオプションキー。デフォルト 360 秒を上書き可能。DPU リブート時の gNMI 接続確認（`gnmi_port` 参照）に間接影響 | `chassisd:720-729` |
| `gnmi_port` 直接参照 | `reboot_smartswitch_helper` が `sonic-db-cli CONFIG_DB hget "DPU|*<dpu_name>" gnmi_port` でポートを取得。欠如時は gNMI 接続確認なしでリブートが進む | `reboot_smartswitch_helper:40-47` |

### SmartSwitch 判定と DashEniFwdOrch 起動分岐

`device_info.is_smartswitch()` は `platform.json` に `"DPUS"` キーが存在するかで判定する:
...
orchagent は `gMySwitchSubType == "SmartSwitch"` のときのみ `DashEniFwdOrch` を起動して CONFIG_DB `DPU` テーブルを読み込む (`orchdaemon.cpp:613-618`)。`DEVICE_METADATA.localhost.subtype` がこの値になるのは SmartSwitch [NPU](../../reference/glossary.md#term-npu) 側のみであり、DPU ノード（`switch_type=dpu`）では `DashEniFwdOrch` は起動しない。

### DPU テーブル書き込み元

SmartSwitch [NPU](../../reference/glossary.md#term-npu) 側では `sonic-config-engine/smartswitch_config.py` が `platform.json` の `DPU` セクションを CONFIG_DB に書き込む。DPU ノード側の `platform.json` の `DPU` セクションは空 dict が典型であり、実際の IP アドレス・ポート番号は minigraph (`smartswitch_config.py:34-37`) から生成される。

非 SmartSwitch 環境では `CONFIG_DB` に `DPU` テーブルが存在せず、`DashEniFwdOrch` も起動しないため `DPU` テーブルは実質的に無効化される。
<!-- /platform -->
...
<!-- glossary-links-injected: f5ea46646200 -->
