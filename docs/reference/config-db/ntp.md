---
title: NTP テーブル群 (Phase A defaults + Phase B ordering + Phase D failure + Phase F side-effects)
description: "NTP / NTP_SERVER / NTP_KEY の各フィールドに対するコード由来の暗黙デフォルト・乖離・dead field・silent drop、書込み順依存、hostcfgd / chrony テンプレート・chronyd-starter.sh の失敗挙動、および /etc/chrony への副次ファイル書込と systemd 経路を網羅した調査ページ。"
area: reference
hard: 0
