...
> 調査日 2026-05-15。ソース: `sonic-swss/orchagent/vrforch.{cpp,h}`、`sonic-swss/orchagent/orch.{cpp,h}`、`sonic-swss/cfgmgr/vrfmgr.cpp`。証跡: `meta/_intermediate/cdb-flow/appl-vrf-pubsub.md`。

### 購読方式: ConsumerStateTable (バッチ pop)

`VRFOrch` は `Orch2` を継承し、コンストラクタで APPL_DB の `DBConnector` と `APP_VRF_TABLE_NAME = "VRF_TABLE"` を渡す[^vrforch_h_g]。`Orch` の基底コンストラクタは DB ID に応じて consumer の種類を切り替えており、**APPL_DB の場合は `ConsumerStateTable` が選択される**[^orch_consumer_g]。CONFIG_DB / STATE_DB / CHASSIS_APP_DB なら `SubscriberStateTable`、それ以外は `ConsumerStateTable` という分岐になっている。
...
したがって `VRF_TABLE` は **`ConsumerStateTable` + `gBatchSize` (デフォルト 128)** で購読される。Redis Lua スクリプトが `VRF_TABLE_KEY_SET` を atomically drain し、`Consumer::execute()` 経由で `getConsumerTable()->pops()` (`orch.cpp:556`) を駆動する。

!!! warning "本ページ「購読者」節の表現について"
    冒頭の概要・購読者節では「`SubscriberStateTable` で購読」と表現しているが、これは正確には **`ConsumerStateTable`** が正しい。APPL_DB 用テーブルは keyspace notification (`SubscriberStateTable`) ではなく `<TABLE>_KEY_SET` を介したバッチ pop で消費される。

### Producer 側: vrfmgrd の ProducerStateTable

`vrfmgrd` (`vrfmgr.cpp:303`) は `m_appVrfTableProducer.set(vrfName, kfvFieldsValues(t))` で書き込む。`ProducerStateTable::set()` は Lua スクリプトにより hash 本体の HSET と `<TABLE>_KEY_SET` への SADD を同一トランザクションで行い、`ConsumerStateTable::pops()` 側と整合する。

### バッチ駆動と dispatch

1. `OrchDaemon` の `Select::select()` が `Consumer` の readiness を検出
2. `Consumer::execute()` が `pops(*entries)` を呼び、Redis から最大 `gBatchSize` 件まで pop
3. pop した KV を `m_toSync` map にマージ (同一 key の上書きで重複削減)
4. `Orch2::doTask(Consumer&)` (`orch.cpp:1226`) が `m_toSync` をループし、`kfvOp(t)` に応じて `VRFOrch::addOperation()` / `delOperation()` (`vrforch.cpp:27, 157`) を 1 件ずつ呼ぶ
5. 各エントリは `Request::parse()` でフィールド型検証 (`request_description` (`vrforch.h:25-40`) に従う) を受けてから処理本体に入る

### VxlanTunnelOrch / EvpnNvoOrch / PortsOrch との連携 (pub/sub ではない)

VNI マッピング処理 (`VRFOrch::updateVrfVNIMap`、`vrforch.cpp:200-247`) は **APPL_DB pub/sub ではなく `gDirectory` レジストリ経由の同期メソッド呼び出し** で他 Orch と連携する。

```cpp
// vrforch.cpp:205-206, 233, 239
EvpnNvoOrch* evpn_orch = gDirectory.get<EvpnNvoOrch*>();
VxlanTunnelOrch* tunnel_orch = gDirectory.get<VxlanTunnelOrch*>();
...
vlan_id = tunnel_orch->getVlanMappedToVni(vni);
gPortsOrch->updateL3VniStatus(vlan_id, true);
```

つまり `VRF_TABLE` の `vni` フィールド変化は次の流れで処理される:

1. APPL_DB `VRF_TABLE` → `ConsumerStateTable` で pop
2. `VRFOrch::addOperation()` 内で `EvpnNvoOrch::getEVPNVtep()` を直接コール (EVPN VTEP が未確立なら処理スキップ)
3. `VxlanTunnelOrch::getVlanMappedToVni(vni)` を直接コールして対応 VLAN を取得
4. `PortsOrch::updateL3VniStatus(vlan_id, true/false)` を直接コールして L3 VNI 状態を更新

`VxlanTunnelOrch` 側は別途 APPL_DB `VXLAN_TUNNEL_MAP_TABLE` を自身の `ConsumerStateTable` で購読しているが、`VRFOrch` とはイベント経路として独立。両者は **Orch 起動順 (`gDirectory` 登録順)** と **VNI ↔ VLAN マップの可用性** によってのみ結合する。

### VNET_TABLE は VRFOrch の購読対象外

APPL_DB `VNET_TABLE` は `VnetOrch` (`orchagent/vnetorch.cpp`) が独立した Executor として購読し、VNET 単位での Virtual Router 作成を独自経路で行う。`VRFOrch` は `VRF_TABLE` 単体のみを購読する。本ページは `VRF_TABLE` のスコープに閉じる。

### バッチサイズ / 優先度

| 項目 | 値 | 出所 |
|------|----|------|
| consumer 種別 | `ConsumerStateTable` | `orch.cpp:1194` |
| バッチサイズ | `gBatchSize` (デフォルト 128、`--batch-size` で上書き可) | orchagent グローバル |
| 優先度 | `default_orch_pri` (省略時) | `orch.h`、`vrforch.h:52-56` で pri 引数未指定 |
| ディスパッチ | `Orch2::doTask(Consumer&)` → `addOperation` / `delOperation` | `orch.cpp:1226` |

[^orch_consumer_g]: `sonic-swss/orchagent/orch.cpp:1188-1195`. <https://github.com/sonic-net/sonic-swss/blob/4305596156d70e9797e8a881b3d19b46de0bce0d/orchagent/orch.cpp>
[^vrforch_h_g]: `sonic-swss/orchagent/vrforch.h:52-56`. <https://github.com/sonic-net/sonic-swss/blob/4305596156d70e9797e8a881b3d19b46de0bce0d/orchagent/vrforch.h>
<!-- /pubsub -->
