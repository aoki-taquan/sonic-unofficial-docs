...
- **順序依存**: portsorch は `gBufferOrch->isPortReady(port)` が `true` になるまでポートを pending 保留する (`portsorch.cpp L4779-4788`)。[BUFFER_PG](../../reference/glossary.md#term-buffer-pg) / BUFFER_QUEUE を PORT 追加と同時か直後に書き込まないと新ポートが準備完了にならない。
