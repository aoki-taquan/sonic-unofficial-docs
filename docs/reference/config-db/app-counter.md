...
`FLEX_COUNTER_TABLE|FLOW_CNT_TRAP` / `|FLOW_CNT_ROUTE` および `FLOW_COUNTER_ROUTE_PATTERN` の SET を起点に、orchagent (`FlexCounterOrch` / `CoppOrch` / `FlowCounterRouteOrch`) が **CONFIG_DB 以外の 3 つの DB** へ副次的に HSET / HDEL を行う。これらは CLI からは直接書き込まず、必ず CONFIG_DB の上記キーが起点になる。

### 副次書込一覧

| 副次 DB | テーブル / key | 書込元 (関数) | トリガ | evidence |
|---|---|---|---|---|
| **COUNTERS_DB** | `COUNTERS_TRAP_NAME_MAP` (HSET trap_name → counter_oid) | `CoppOrch::bindTrapCounter()` | `FLOW_CNT_TRAP=enable` で trap ごとに HSET、disable で `hdel` | `copporch.cpp:196, 1452-1456, 1494-1496` |
| **COUNTERS_DB** | `COUNTERS_ROUTE_NAME_MAP` (HSET `<vrf>\|<prefix>` → counter_oid) | `FlowCounterRouteOrch::doTask(SelectableTimer&)` (`mPrefixToCounterTable->set`) | `FLOW_CNT_ROUTE=enable` + pattern マッチ route 検出時。1 秒タイマー経由 | `flowcounterrouteorch.cpp:33, 150-153, 921-922` |
| **COUNTERS_DB** | `COUNTERS_ROUTE_TO_PATTERN_MAP` (HSET route → 親 pattern) | 同上 (`mPrefixToPatternTable->set`) | route bind 時 | `flowcounterrouteorch.cpp:34, 155-158, 921` |
| **COUNTERS_DB** | `COUNTERS:<counter_oid>` (HSET `SAI_COUNTER_STAT_PACKETS`/`_BYTES`) | syncd FlexCounter (orchagent 経由ではない) | 10 秒周期 polling | `flow_counter_handler.cpp:10-13` |
| **STATE_DB** | `FLOW_COUNTER_CAPABILITY_TABLE\|route` (`support: true/false`) | `FlowCounterRouteOrch` ctor | orch 起動時 1 回。SAI capability 問合結果を保存 | `flowcounterrouteorch.cpp:166-179` |
| **FLEX_COUNTER_DB** | `FLEX_COUNTER_GROUP_TABLE\|HOSTIF_TRAP_FLOW_COUNTER` / `\|ROUTE_FLOW_COUNTER` | `FlexCounterOrch` → `setFlexCounterGroupOperation()` / `setFlexCounterGroupPollInterval()` | `FLEX_COUNTER_STATUS` / `POLL_INTERVAL` 変更時 | `flexcounterorch.cpp:202-214, 380-392`, `saihelper.cpp:868-885,918-962` |
| **FLEX_COUNTER_DB** | `FLEX_COUNTER_TABLE:<counter_oid>` (per-OID stat list) | `FlexCounterManager::setCounterIdList()` | trap / route → counter 紐付け時 | `flex_counter_manager.cpp:200-260` |

### COUNTERS_DB 書込の起点（コード抜粋）

```cpp
// copporch.cpp:1452-1456 - trap counter name map
...
m_counter_table->set("", nameMapFvs);   // COUNTERS_DB / COUNTERS_TRAP_NAME_MAP
```

```cpp
// flowcounterrouteorch.cpp:150-158 - route counter name maps
if (!prefixToCounterMap.empty())
    mPrefixToCounterTable->set("", prefixToCounterMap);  // COUNTERS_ROUTE_NAME_MAP
if (!prefixToPatternMap.empty())
    mPrefixToPatternTable->set("", prefixToPatternMap);  // COUNTERS_ROUTE_TO_PATTERN_MAP
```

### STATE_DB capability テーブル

```cpp
// flowcounterrouteorch.cpp:174-178 - 起動時 1 回のみ
...
capability_table.set(FLOW_COUNTER_ROUTE_KEY, fvs);  // STATE_DB / FLOW_COUNTER_CAPABILITY_TABLE|route
```

trap 側 (`FLOW_CNT_TRAP`) には capability 事前判定がないため、`STATE_DB FLOW_COUNTER_CAPABILITY_TABLE` には **trap 側エントリは書かれない**。

### クリア / 残置リスク

| ケース | 副次 DB の挙動 |
|---|---|
| `FLOW_CNT_TRAP=disable` | `COUNTERS_TRAP_NAME_MAP` は `unbindTrapCounter()` で `hdel`、SAI `remove_counter` 失敗時は OID リーク (`flow_counter_handler.cpp:32-38`) |
| `FLOW_CNT_ROUTE=disable` / pattern DEL | `COUNTERS_ROUTE_NAME_MAP` / `COUNTERS_ROUTE_TO_PATTERN_MAP` は `removeRouteFlowCounterFromDB()` で `hdel` (`flowcounterrouteorch.cpp:921-922`) |
| orchagent プロセス再起動 | `STATE_DB FLOW_COUNTER_CAPABILITY_TABLE\|route` のみ再評価。CONFIG_DB を起点に他副次テーブルは順次再生成 |
| SAI capability false 確定後 | `FLOW_CNT_ROUTE=enable` を書いても **どの副次 DB にも書込なし** (完全 no-op) |

!!! note "ユーザー観測点"
    - `redis-cli -n 2 HGETALL COUNTERS_TRAP_NAME_MAP` (COUNTERS_DB = idx 2) で trap → OID 対応を確認
    - `redis-cli -n 2 HGETALL COUNTERS_ROUTE_NAME_MAP` で route → OID 対応を確認
    - `redis-cli -n 6 HGETALL "FLOW_COUNTER_CAPABILITY_TABLE|route"` (STATE_DB = idx 6) で route capability を確認
