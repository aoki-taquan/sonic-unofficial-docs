...
> 根拠: `sonic-net/sonic-gnmi:telemetry/telemetry.go` (ref: eb635b7679b260c3fd0786a6d0734fc8e82c9a22); `sonic-net/sonic-gnmi:pkg/bypass/bypass.go` (ref: eb635b7679b260c3fd0786a6d0734fc8e82c9a22)

### Go フラグデフォルト（CONFIG_DB 非管理）
...
| `config_table_name` フラグデフォルト | `""` (空文字列) | gNMI クライアント証明書 CN 認証テーブル名。`user_auth=cert` 時は `gnmi-native.sh` が `GNMI_CLIENT_CERT` を明示設定 | `telemetry.go:177` |

### TLS / gRPC セキュリティ定数

CONFIG_DB から変更不可。コード内にリテラルで固定。
...
### bypass.go — CVL バイパス許可 SKU プレフィックス

SmartSwitch 向け gNMI Set RPC で CVL バリデーションをスキップする SKU のハードコードリスト。CONFIG_DB / YANG での管理なし。
...
| `defaultRedisSocket` | `/var/run/redis/redis.sock` | bypass 内部の Redis 接続用 Unix ソケット | `bypass.go:43` |
...
!!! warning "AllowedSKUPrefixes は変更不可"
    `AllowedSKUPrefixes` はコード変更なしに拡張できない。新 SKU を追加するには `bypass.go` の再ビルドが必要。

!!! note "cert_crl_dir の注意"
    `cert_crl_dir` (`/mtls/crl`) は `enable_crl=true` 時のみ参照される。ディレクトリが空のまま `enable_crl=true` にすると、CRL ダウンロードを試みるが全接続が拒否される。

<!-- evidence: sonic-net/sonic-gnmi:telemetry/telemetry.go:175,177,183,184,203,209,210,482,547 (ref: eb635b7679b260c3fd0786a6d0734fc8e82c9a22); sonic-net/sonic-gnmi:pkg/bypass/bypass.go:33-45 (ref: eb635b7679b260c3fd0786a6d0734fc8e82c9a22) -->
<!-- /constants -->
