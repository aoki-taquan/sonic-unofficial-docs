...
`BUFFER_PORT_INGRESS_PROFILE_LIST` は **2 段階中継** で CONFIG_DB から SAI へ到達する。

```
CONFIG_DB
  ↓ SubscriberStateTable (keyspace notification)
buffermgrd (buffermgrdyn)
  ↓ ProducerStateTable → PUBLISH BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE_CHANNEL@0
APPL_DB
  ↓ ConsumerStateTable → SUBSCRIBE BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE_CHANNEL@0
BufferOrch (orchagent)
  ↓ sai_port_api->set_ports_attribute() [Bulk SET]
SAI / ASIC
```

### CONFIG_DB → buffermgrd (`SubscriberStateTable`)

`buffermgrd.cpp:181` が `TableConnector(&cfgDb, CFG_BUFFER_PORT_INGRESS_PROFILE_LIST_NAME)` で本テーブルを登録する。`Orch::addConsumer()` (`orch.cpp:1186-1196`) は CONFIG_DB (id≠0) に対して **`SubscriberStateTable`** を生成し、Redis keyspace notification

```
PSUBSCRIBE __keyspace@{config_db_id}__:BUFFER_PORT_INGRESS_PROFILE_LIST|*
```

を購読する。変化があると `buffermgrdyn.cpp:447` に登録された `handleBufferPortIngressProfileListTable` が呼ばれ、`handleBufferObjectTables` 経由で `handleSingleBufferPortIngressProfileListEntry` が各ポートに対して実行される。

### buffermgrd → APPL_DB (`ProducerStateTable`)

`buffermgrdyn.cpp:47` で宣言された `ProducerStateTable(applDb, APP_BUFFER_PORT_INGRESS_PROFILE_LIST_NAME)` を使い `BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE` に書き込む。`ProducerStateTable::set()` は key を `*_KEY_SET` ハッシュに格納後、`BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE_CHANNEL@0` に **PUBLISH** する。

### APPL_DB → BufferOrch (`ConsumerStateTable`)

`orchdaemon.cpp:391` が `APP_BUFFER_PORT_INGRESS_PROFILE_LIST_NAME` を `buffer_tables` に追加し、`BufferOrch(m_applDb, ...)` へ渡す。APPL_DB (id=0) では `addConsumer()` が **`ConsumerStateTable`** を生成し、

```
SUBSCRIBE BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE_CHANNEL@0
```

を購読する。`bufferorch.cpp:77/80` でハンドラを登録:

| ハンドラ | 用途 |
|---------|------|
| `processIngressBufferProfileList` | 単一エントリの SAI 属性更新 |
| `processIngressBufferProfileListBulk` | 複数ポートを `sai_port_api->set_ports_attribute()` で一括 SET |

Bulk SET は **DEL → SET** の順で処理され、`SAI_BULK_OP_ERROR_MODE_IGNORE_ERROR` で部分失敗を許容する。失敗ポートは `consumer.m_toSync` に再投入されリトライ。

### ResponsePublisher — 上り ack なし

`bufferorch.cpp` の ingress profile list ハンドラは `m_publisher.publish()` を呼ばない。ResponsePublisher は BUFFER_POOL / BUFFER_PROFILE のみ使用する。
...
| BufferOrch → APPL_STATE_DB | `ResponsePublisher` | **なし** |

evidence: `buffermgrdyn.cpp:47,447,3566`, `buffermgrd.cpp:181`, `bufferorch.cpp:53,77,80,1796-1848`, `orchdaemon.cpp:391`
<!-- /pubsub -->
