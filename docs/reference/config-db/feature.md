...
`telemetry` / `frr_bmp` は `enable_feature()` / `disable_feature()` の冒頭で即 return する（`featured:469-471, 517-519`）。CONFIG_DB の state 変更が systemd に適用されず、[STATE_DB](../../reference/glossary.md#term-state_db) も更新されない（`set_feature_state()` 以前に return するため `FEATURE|<name>.state` フィールドは変化しない）。
