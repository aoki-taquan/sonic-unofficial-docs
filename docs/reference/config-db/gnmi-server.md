...
これら 2 つのフィールドは GNMI テーブルに対応するフィールドを持たず、スクリプトが `sonic-db-cli CONFIG_DB hget` で直接取得する。どちらもコンテナ起動時 1 回の読み取りであり、設定変更の反映にはコンテナ再起動が必要。詳細は [VRF](../../reference/glossary.md#term-vrf) / [SmartSwitch](../../reference/glossary.md#term-smartswitch) 自動連携 を参照。
...
| VRF (`MGMT_VRF_CONFIG\|vrf_global.mgmtVrfEnabled`) | VRF 対応機種全般で共通 | `gnmi-native.sh:95-98` で `--vrf mgmt` を付与。特定 [ASIC](../../reference/glossary.md#term-asic) への依存なし |
...
<!-- glossary-links-injected: 5919a9cdd94e -->
