...
| フィールド | 型 | [APPL_DB](../../reference/glossary.md#term-appl_db) での扱い | 説明 |
|-----------|----|----------------|------|
| `tagging_mode` | string (`tagged`/`untagged`/`priority_tagged`) | CONFIG_DB のフィールドをそのまま転送 | CONFIG_DB 省略時は [vlanmgrd](../../reference/glossary.md#term-vlanmgrd) が `"untagged"` で補完。portsorch も同じく `"untagged"` fallback |
...
| VLAN_TABLE DEL: [FDB](../../reference/glossary.md#term-fdb) / ref count / メンバ / VNI / host_intf 残存 | `portsorch.cpp:7427-7461` | `removeVlan()` が `return false` → 外側 (`L5844-5847`) で `it++` retry。fdborch / intfsorch / vxlanorch の削除待ち |
