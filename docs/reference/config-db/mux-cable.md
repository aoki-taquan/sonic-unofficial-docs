...
| `neighbor_mode` | `prefix-route` | サーバ IP を prefix-based route として処理。動的変更は [muxorch](../../reference/glossary.md#term-muxorch) で WARN (再起動が必要) |
...
| `neighbor_mode` | `host-route` | SAI が `SAI_NEIGHBOR_ENTRY_ATTR_NO_HOST_ROUTE` 非対応 → `"prefix-route"` を指定しても silent に `host-route` 動作 ([muxorch](../../reference/glossary.md#term-muxorch).cpp:2240) | プラットフォーム依存 silent 無視 |
| `server_ipv4` | — (optional) | 欠落時に `request.getAttrIpPrefix()` が `std::out_of_range` 例外 → [orchagent](../../reference/glossary.md#term-orchagent) 障害 ([muxorch](../../reference/glossary.md#term-muxorch).cpp:2206) | **YANG optional vs 実装 mandatory** |
