...
### H-5. P2P / P2MP / MP2MP decap term — SAI capability クエリなし

`tunneldecaporch` は P2P / P2MP / MP2MP の SAI capability を事前クエリせず、`sai_tunnel_api->create_tunnel_term_table_entry()` を直接呼び出す。非対応プラットフォームでは SAI エラーが返り `handleSaiCreateStatus()` でタスク失敗として処理される（リトライなし）。

- **Dual-ToR MuxTunnel**: P2P (`src_ip = PEER_SWITCH.address_ipv4`) を使用。
- **通常 IPinIP decap**: `src_ip` 未設定時は P2MP (ワイルドカード) decap term を作成。
- **Subnet decap (IPINIP_SUBNET)**: MP2MP term が必須。`is_subnet_decap_term && term_type != MP2MP` はエラー扱い (`tunneldecaporch.cpp` L446-448`)。
