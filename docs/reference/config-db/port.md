...
> **ソース精読メモ**: `APPL_DB PORT_TABLE_TX_READY` という独立テーブルは `portsorch.cpp` に存在しない。`host_tx_ready` フィールドは **STATE_DB の `PORT_TABLE`** (`m_portStateTable`) に書き込まれる。また **APPL_STATE_DB** は portsorch の PORT 処理では使用されない（`ResponsePublisher::publish()` が PORT テーブルに対して呼ばれていないため）。
