...
!!! warning "裏取りステータス: HLD-only"
    本 HLD は SNMP 設定を `/etc/sonic/snmp.yml` から CONFIG_DB に移行する設計を述べる。`config snmp ...` / `show run snmp ...` CLI 系の sonic-utilities への取り込み状況、`docker-snmp` の `snmpd.conf.j2` 改修と Python 変換スクリプトの現行 master 取り込みは未確認。
