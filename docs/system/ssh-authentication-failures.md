...
[SONiC](../reference/glossary.md#term-sonic) デバイスへ SSH 接続する際、`Too many authentication failures` エラーが発生して接続できない場合がある。
...
OpenSSH クライアントは、接続時にローカルの SSH エージェントや `~/.ssh/` 以下に登録されている**すべての秘密鍵**を順番に試行する。秘密鍵の数が `MaxAuthTries`（sshd の設定値、デフォルト 6）を超えると、サーバが接続を切断する。
