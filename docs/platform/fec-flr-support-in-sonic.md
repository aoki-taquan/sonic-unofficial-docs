...
| `FEC_FLR_R_SQUARED` | predicted の決定係数（R²） |
...
!!! diff "HLD と実装の差分"
    ...
    | `SAI_PORT_STAT_IF_IN_FEC_CODEWORD_ERRORS_S0..S15` の利用 | 必須 | `sonic-swss/crates/countersyncd/src/sai/saiport.rs` L766-782 で S0〜S16 を列挙（FLR 計算は S0..S15 の 16 ビンを使用。S16 は SAI 側に拡張定義として存在するが FLR 算出対象外）、L1144- で文字列マッピング | ✓ 実装済み |
    ...
    **回避策 / 対応方法**:
    ...
    - コアロジックと表示は取り込み済み: `sonic-swss/orchagent/port_flr.lua` L1-460（FLR 計算は S0..S15 の 16 ビンを使用）、`sonic-swss/crates/countersyncd/src/sai/saiport.rs` L766-782 で SAI_PORT_STAT_IF_IN_FEC_CODEWORD_ERRORS_S0..S16 を列挙（S16 は SAI 側の拡張定義で FLR 算出には未使用）、`sonic-utilities/utilities_common/portstat.py` L50/L271-273/L671-673 で表示列追加。
