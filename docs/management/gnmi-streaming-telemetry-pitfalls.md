...
dial-in ストリーミングでは、gNMI サーバ（スイッチ側）がサブスクリプションに対するデータを内部キューにバッファリングし、コレクターに送信する。コレクターの受信速度がデータ生成速度を下回ると、キューが際限なく積み上がり、メモリを圧迫する。
...
キューが一定以上膨らむと、gNMI サーバは接続に対して `RESOURCE_EXHAUSTED` エラーを返して接続を切断する。これはメモリ保護のための意図的な動作である。
...
# Docker コンテナのメモリ使用量を確認（gnmi コンテナ）
docker stats gnmi --no-stream
```
