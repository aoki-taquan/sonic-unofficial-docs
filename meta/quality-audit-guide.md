...
### 5.1 `monitor: not_implemented`

HLD が提案段階で master に対応コードが無いため、通常の「設定例 / 制限 / 確認コマンド」は本来存在しない。サブ軸は以下のように読み替える。

| サブ軸 | 読み替え後の評価 |
|--------|----------------|
| **6a** 設定例 | **N/A**（実装が無い以上、提示する設定例は存在しない。記載しないことを減点しない） |
| **6b** 制限事項 | 「未実装である旨」が本文に明示されていれば**満点扱い**（提案範囲・採用見送りの理由・着手前である旨など、ユーザに「使えない」と分かる記述があれば 5 とする） |
| **6c** 確認コマンド | 「現状で確認方法が無い旨」が明示されていれば**満点扱い**（`show` 系で観測できない・該当テーブルが空である等、確認手段が無い事実を述べていれば 5 とする） |

軸 6 全体の集計は 6b / 6c の 2 サブ軸平均（6a は N/A 除外）で算出する。
...
```
[not_implemented]      軸 6: 5.00 (6a=N/A, 6b=5, 6c=5)
