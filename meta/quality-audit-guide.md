...
## 4. サブ軸定義（軸 5 / 軸 6、round 33 から試行）

audit round 32 の改善提言を受け、軸 5 (可読性) と軸 6 (完結性) を 3 つのサブ軸に分割して評価する運用を **round 33 から試行** する。各サブ軸 5 点満点、3 サブ軸の **単純平均** で当該軸のスコアを算出する（小数第 2 位四捨五入）。

過去 round (round 32 以前) は本サブ軸を適用せず、軸単位の総合 5 点評価が継続して有効。前向き運用とし、既存 audit ファイルは遡及修正しない。
...
サブ軸を適用した round では、各ページの評価表に軸スコアと並んで `(5a/5b/5c)` `(6a/6b/6c)` の内訳を併記する。例:

> 軸 5: 4.67 (5a=5, 5b=4, 5c=5) / 軸 6: 4.00 (6a=5, 6b=3, 6c=4)

`page_kind: chapter-index` の軸 6 は引き続き N/A とし、サブ軸も評価しない。`verification: discrepancy-found` ページの軸 6 は §1.2 / §2 の「乖離説明の整理度」基準を優先し、サブ軸 6a-6c は副次情報として併記のみ可（軸スコアの算出には用いない）。
