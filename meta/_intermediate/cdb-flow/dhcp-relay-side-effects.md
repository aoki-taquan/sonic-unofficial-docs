...
追加ソース: `sonic-dhcp-relay/dhcp6relay/src/relay.cpp:264-304, 1342-1401`
...
`dhcp6relay` は `DHCP_RELAY` に登録された VLAN ごとに `STATE_DB::DHCPv6_COUNTER_TABLE|<vlan>` を初期化・更新する（relay.cpp:273-304）。

---

## 1. STATE_DB 読み取り（書き込みなし、dhcprelayd）
...
## 2. STATE_DB 書き込み（dhcp6relay）

`dhcp6relay` は起動時に `DHCPv6_COUNTER_TABLE|<vlan>` を全カウンタ `"0"` で初期化し、パケット受信ごとにインクリメントする。

```c
// relay.cpp:273-304
void initialize_counter(state_db, ifname) {
    clear_counter(state_db);  // DEL "DHCPv6_COUNTER_TABLE|<vlan>|*"
    state_db->hset(table_name, msg_type, "0");  // 全 msg_type を 0 で初期化
}

void increase_counter(state_db, ifname, msg_type) {
...
| 書き込みキー | タイミング | 値 |
|------------|----------|-----|
| `DHCPv6_COUNTER_TABLE\|<vlan>\|Solicit` 等 15 種別 | パケット受信ごと | 累積カウント (string) |
| 全カウンタ | `dhcp6relay` 起動時 | `"0"` にリセット |

`sendto()` 失敗時はカウンタ加算されない（sender.cpp:21-27）。

---

## 3. dhcrelay プロセス起動・停止（subprocess 副作用、dhcprelayd）
...
## 5. supervisord 設定ファイル（docker_init.sh が生成）
...
## 6. COUNTERS_DB クリア（start.sh が実施）
...
| DB | 理由 |
|----|------|
| STATE_DB への書き込み (dhcprelayd) | dhcprelayd は読み取りのみ。書き込みは dhcpservd が担当 |
| APPL_DB | 書き込みなし |
...
| コード | 内容 |
|--------|------|
| `relay.cpp:18, 264-304` | `initialize_counter` / `increase_counter`: STATE_DB DHCPv6_COUNTER_TABLE 書込 |
| `relay.cpp:1342-1401` | `clear_counter` + `initialize_counter` 呼び出し（lla_check_callback から） |
