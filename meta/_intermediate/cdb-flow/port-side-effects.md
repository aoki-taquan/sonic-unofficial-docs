...
| APPL_DB / `PORT_TABLE_TX_READY` | **使用なし** | このテーブル名は portsorch.cpp に存在しない。`host_tx_ready` フィールドは STATE_DB の `PORT_TABLE` に書き込まれる (`m_portStateTable.hset(..., "host_tx_ready", ...)`) |
| STATE_DB / `PORT_TABLE` | 使用あり (`m_portStateTable`) | supported_speeds, supported_fecs, host_tx_ready, link_training_status 等 |
| APPL_STATE_DB | **使用なし** | portsorch は `ResponsePublisher` を基底クラス (`Orch`) 経由で持つが、PORT テーブルの処理に対して `m_publisher.publish()` を呼び出しておらず、APPL_STATE_DB への書き込みは発生しない |
