...
BUFFER_QUEUE の副次 DB 書込は COUNTERS_DB（4 マップ）と FLEX_COUNTER_DB（最大 3 グループ）への queue カウンタ登録。非 VOQ かつ `isCreateOnlyConfigDbBuffers()` が true の場合のみ発生。zero profile 時はスキップ。
