...
### ACL_ENTRY / ACL_COUNTER: sai_acl_api 経由

`CRM_ACL_ENTRY` / `CRM_ACL_COUNTER` は ACL テーブルごとに `sai_acl_api->get_acl_table_attribute()` で `SAI_ACL_TABLE_ATTR_AVAILABLE_ACL_ENTRY` / `SAI_ACL_TABLE_ATTR_AVAILABLE_ACL_COUNTER` を取得（`crmResSaiAvailAttrMap` L86-87）。`sai_switch_api` ではなく `sai_acl_api` を使う点が他リソースと異なる。
