...
- **順序依存 (SET)**: `MGMT_VRF_CONFIG|vrf_global` に `mgmtVrfEnabled=true` を書き込む場合、`m_stateVrfTable` に "mgmt" エントリが既に存在する場合は SET がスキップされる（重複 SET 無効化、`vrfmgr.cpp:263-265`）。再度有効化したい場合は DEL → SET の順序が必要。
