# DASH_ROUTING_TYPE_TABLE — Phase G: ZMQ / ZmqConsumerStateTable / ProducerStateTable

対象ページ: `docs/reference/config-db/dash-routing-types.md`
調査日: 2026-05-17
Evidence:
- `sonic-swss/orchagent/dash/dashorch.cpp` (L60-61, L473, L1334-1360)
- `sonic-swss/orchagent/zmqorch.cpp` (L41-78) および `zmqorch.h`
- `sonic-swss/orchagent/orchdaemon.cpp` (L1322–1351)
- `sonic-swss-common/common/zmqserver.h` (ORCH_ZMQ_PORT, MQ_POLL_TIMEOUT)
...
`DASH_ROUTING_TYPE_TABLE` の変更通知は **Redis keyspace notification ではなく ZeroMQ (ZMQ) メッセージ** で実装されている。
`DashOrch` は `ZmqOrch` (`zmqorch.cpp:60-61`) を継承し、`ZmqConsumerStateTable` を通じて ZMQ PUSH/PULL パターンで受信する。
`SubscriberStateTable` / `NotificationConsumer` / Redis PSUBSCRIBE は一切使用しない。

`DASH_ROUTING_TYPE_TABLE` は `DashOrch` が受け持つ `dash_tables` ベクタ
(`orchdaemon.cpp:1342-1348`) の一員として登録され、`DashOrch` コンストラクタ
(`dashorch.cpp:60-61`) が `ZmqOrch(db, tableName, zmqServer)` に委譲する。
`ZmqOrch::addConsumer()` (`zmqorch.cpp:59-78`) が `ZmqConsumerStateTable` を生成し、
ZmqServer に handler を登録する。
...
### 1. orchagent 起動時 — `DpuOrchDaemon::init()` (orchdaemon.cpp:1322-1351)
...
DpuOrchDaemon::init()
  ...
  └─ DashOrch(m_dpu_appDb, dash_tables, m_dpu_appstateDb, dash_zmq_server)
       └─ ZmqOrch::ZmqOrch(db, tableNames, zmqServer)
            └─ addConsumer(db, "DASH_ROUTING_TYPE_TABLE", default_pri, zmqServer)
                 └─ ZmqConsumerStateTable(db, "DASH_ROUTING_TYPE_TABLE", *zmqServer, gBatchSize, pri, dbPersistence=true)
                      └─ zmqServer.registerMessageHandler("DPU_APPL_DB", "DASH_ROUTING_TYPE_TABLE", this)
                      └─ m_asyncDBUpdater = AsyncDBUpdater(db, "DASH_ROUTING_TYPE_TABLE")  ← DB persistence 有効
```
...
DASH コントローラ (gNMI / sonic-mgmt-common 等)
  ...
  └─ ZmqProducerStateTable(dpu_app_db, "DASH_ROUTING_TYPE_TABLE", zmq_client, dbPersistence=true)
       └─ set(routing_type_key, [("pb", protobuf.SerializeToString())])  ← SET
       └─ del(routing_type_key)                                          ← DEL
```

gNMI SetRequest → `sonic-mgmt-common` が `dash::route_type::RouteType` Protobuf にエンコード →
`ZmqProducerStateTable::set()` → `ZmqClient::sendMsg()` → ZMQ PUSH ソケット経由で orchagent の ZmqServer へ送信。

`dbPersistence=true` の場合、`ZmqProducerStateTable` は送信と同時に `DPU_APPL_DB` の
`DASH_ROUTING_TYPE_TABLE` にも Redis 書き込みを行う (`AsyncDBUpdater` 経由で非同期)。
...
ZmqServer::mqPollThread()  [バックグラウンドスレッド]
  ...
  └─ findMessageHandler("DPU_APPL_DB", "DASH_ROUTING_TYPE_TABLE") → ZmqConsumerStateTable*
...
受信した SET/DEL は ZMQ 経由で処理されると同時に、非同期で `DPU_APPL_DB` の
`DASH_ROUTING_TYPE_TABLE` にも書かれる。
orchagent 再起動時は `warmRestoreAndSyncUp()` → `bake()` が APP_DB の既存エントリを
`m_toSync` に積み直し、3 イテレーション `doTask()` を実行する。
これにより、コントローラの再送なしで routing type 設定が再適用される。

---

## Redis との関係

`DASH_ROUTING_TYPE_TABLE` の変更は **ZMQ 経由のみ** で orchagent に到達する。
Redis の ProducerStateTable チャンネル (`DASH_ROUTING_TYPE_TABLE_CHANNEL@<db_id>`) は使用されない。
ただし DB persistence により `DPU_APPL_DB` の `DASH_ROUTING_TYPE_TABLE` には SET/DEL の内容が
非同期書き込みされるため、`redis-cli` や `sonic-db-cli` で参照することは可能（orchagent の入力ではなくログ的役割）。
...
dash_zmq_server = nullptr
  └─ ZmqOrch::addConsumer()
       └─ ConsumerStateTable(db, "DASH_ROUTING_TYPE_TABLE", gBatchSize, pri)
            └─ Redis SUBSCRIBE DASH_ROUTING_TYPE_TABLE_CHANNEL@<db_id>
```
...
| SubscriberStateTable | 不使用 |
| NotificationConsumer | 不使用 |
| DPU_APPL_STATE_DB 書き戻し | あり (`writeResultToDB` / `removeResultFromDB`) |
