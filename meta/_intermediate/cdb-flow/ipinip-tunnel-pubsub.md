# TUNNEL テーブル (IPinIP) — 通信メカニズム (Phase G) 解析メモ

対象: `CONFIG_DB` の `TUNNEL` テーブル（Dual-ToR IPinIP トンネル定義）。
ソース: `sonic-swss/orchagent/tunneldecaporch.cpp`, `orchdaemon.cpp`
...
`tunnelmgrd` (cfgmgr) が CONFIG_DB `TUNNEL` → APPL_DB `TUNNEL_DECAP_TABLE` / `TUNNEL_DECAP_TERM_TABLE` へ変換し、orchagent はこの APPL_DB 側を `ConsumerStateTable` で受け取る二段構成。
...
- テーブル名によるディスパッチで各 sub-handler へ委譲。

## 4. SAI tunnel_api — 呼び出しパターン
...
`docs/reference/config-db/tunnel.md` には既に `<!-- pubsub -->` ... `<!-- /pubsub -->` ブロックが存在するため、本 Phase G では tunnel.md への追記をスキップした。
