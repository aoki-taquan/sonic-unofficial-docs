...
## FRR vtysh コマンド（BGP_GLOBALS の変更で発行される代表コマンド）

`frrcfgd.py:1784-1821` の `global_key_map` から生成:

```
vtysh -c "configure terminal"
  -c "router bgp <local_asn> [vrf <vrf_name>]"
  -c "bgp router-id <router_id>"
  -c "bgp graceful-restart"
  -c "bgp graceful-restart restart-time <gr_restart_time>"
  -c "bgp graceful-restart stalepath-time <gr_stale_routes_time>"
  -c "bgp log-neighbor-changes"
  -c "bgp always-compare-med"
  -c "bgp bestpath as-path multipath-relax [as-set]"
  -c "bgp bestpath compare-routerid"
  -c "bgp fast-external-failover"
  -c "bgp graceful-shutdown"
  -c "timers bgp <keepalive> <holdtime>"
  -c "bgp max-med on-startup <max_med_time> <max_med_val>"
  -c "bgp confederation identifier <confed_id>"
  -c "bgp confederation peers <confed_peers>"
  ...
```

DELETE 時（`data is None`）は `no router bgp <asn> [vrf <vrf>]` を送出（`frrcfgd.py:2458-2460`）。

## bgpcfgd の BGP_GLOBALS 取り扱い

`bgpcfgd` は `BGP_GLOBALS` テーブルを直接購読しない（`main.py:71-120` に `BGP_GLOBALS` 対応 Manager なし）。起動時に `sonic-cfggen` が生成した `bgpd.conf` を FRR に流し込む方式をとる。`managers_bgp.py` の `update_state_db()` は `BGP_NEIGHBOR` / `BGP_NEIGHBOR_AF` のハンドラからのみ呼ばれる（L239, L353, L443, L487）。

## 出力先サマリ

```
BGP_GLOBALS (CONFIG_DB)
  ↓ frrcfgd (bgp_global_handler)
  FRR vtysh → bgpd (プロセス内設定のみ)
  → STATE_DB 書込: なし
  → COUNTERS_DB 書込: なし
  → APPL_DB 書込: なし
```
...
- `frrcfgd.py:1784-1821` `global_key_map` — BGP_GLOBALS フィールドと vtysh コマンドのマッピング
- `frrcfgd.py:2685-2731` BGP_GLOBALS 分岐 — `__update_bgp()` 内処理、DB 書込なし
