# BGP_GLOBALS_AF — 実コンテナ動作トレース (Direction B)
...
| 3. APPL→SAI | なし (FRR BGP の AF レベル設定) |
| 4. タイミング+副作用 | 変化検知後 FRR に AF の global 設定コマンドを発行。既存セッションへの影響は AF の再ネゴシエーションを要する場合がある。... |
