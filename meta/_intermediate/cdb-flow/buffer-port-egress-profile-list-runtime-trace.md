# BUFFER_PORT_EGRESS_PROFILE_LIST — 実コンテナ動作トレース (Direction B)
...
| 2. CFG→APPL 翻訳 | `APP_BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE` に書き込み |
| 3. APPL→SAI | `sai_port_api` — ポートの egress バッファプロファイルリストをバインド |
