...
`stopWPASupplicant(pid)` で `SIGTERM` → `unconfigureMACsec()` が `interface_remove` を送信。失敗時は最大 3 回リトライ（L862 付近の `MAX_INTERFACE_REMOVE_RETRIES`）。
...
- `MACSEC_PROFILE` エントリを削除しようとしたとき、同名プロファイルを使用中のポートが存在する場合は `task_failed` が返却され削除は拒否される（L460-469）
- プロファイル更新時（`loadProfile` の `update()` 呼び出し）は、当該プロファイルを使用中の全ポートに対して MACsec を再設定する（L417-431）
