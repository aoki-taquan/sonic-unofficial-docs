# BUFFER_PORT_EGRESS_PROFILE_LIST — 通信メカニズム (Phase G) 中間調査

対象ページ: `docs/reference/config-db/buffer-port-egress-profile-list.md`

## 1. subscribe 経路の全体像

`BUFFER_PORT_EGRESS_PROFILE_LIST` テーブルは **2 段階中継**:
...
`sonic-swss/cfgmgr/buffermgrdyn.cpp` L448:

```cpp
m_bufferTableHandlerMap.insert(buffer_handler_pair(
    CFG_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME,
    &BufferMgrDynamic::handleBufferPortEgressProfileListTable));
```
...
→ `BUFFER_PORT_EGRESS_PROFILE_LIST` に対して **`SubscriberStateTable`** を生成。
...
m_bufferSingleItemHandlerMap.insert(buffer_single_item_handler_pair(
    CFG_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME,
    &BufferMgrDynamic::handleSingleBufferPortEgressProfileListEntry));
```

`handleBufferPortEgressProfileListTable` は `handleBufferObjectTables(tuple, CFG_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME, false)` に委譲し (`buffermgrdyn.cpp:3571`)、コンマ区切りポートキーを分解して各ポートに `handleSingleBufferPortEgressProfileListEntry` を呼ぶ (`L3536-3547`)。

### static model (`buffermgr.cpp`)

static model は `BUFFER_PORT_EGRESS_PROFILE_LIST` を同様に SubscriberStateTable で購読し CONFIG_DB 値をそのまま APPL_DB に書く（direction / profile 存在検証なし）。ただし `DEVICE_METADATA.buffer_model == "dynamic"` の環境では `L476-480` の早期 return によりスキップされる。
...
`APP_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME` = `"BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE"` に **`ProducerStateTable`** で書き込む。

`ProducerStateTable::set()` は key を `BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE_KEY_SET` ハッシュに格納し、`BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE_CHANNEL@0` に PUBLISH する。

## 4. APPL_DB → BufferOrch (ConsumerStateTable)

`sonic-swss/orchagent/orchdaemon.cpp` L386-L394（抜粋）:
...
vector<string> buffer_tables = {
    ...
    APP_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME   // ← ここで登録
};
...
```
SUBSCRIBE BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE_CHANNEL@0
```

`ConsumerStateTable::pops()` が Lua SCRIPT で `BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE_KEY_SET` からエントリを取り出す（最大 `DEFAULT_POP_BATCH_SIZE = 128` keys/回）。

### ハンドラ登録 (`bufferorch.cpp` L78/L81)

```cpp
m_bufferHandlerMap.insert(buffer_handler_pair(
    APP_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME,
    &BufferOrch::processEgressBufferProfileList));
m_bufferFlushHandlerMap.insert(buffer_flush_handler_pair(
    APP_BUFFER_PORT_EGRESS_PROFILE_LIST_NAME,
    &BufferOrch::processEgressBufferProfileListBulk));
```

`processEgressBufferProfileList` → SAI `sai_port_api` `SAI_PORT_ATTR_QOS_EGRESS_BUFFER_PROFILE_LIST` を更新。

## 5. ResponsePublisher — BUFFER_PORT_EGRESS_PROFILE_LIST には上り ack なし

`orch.h` L382 に `ResponsePublisher m_publisher{"APPL_STATE_DB"}` を持つが、`bufferorch.cpp` の `processEgressBufferProfileList` / `processEgressBufferProfileListBulk` は `m_publisher.publish()` を呼ばない。
...
void BufferOrch::doTask()
{
    ...
    for (auto &it : m_consumerMap) {
        ...
        consumer->drain();   // ← BUFFER_PORT_EGRESS_PROFILE_LIST もここで処理
    }
}
```

`BUFFER_PORT_EGRESS_PROFILE_LIST` は pool / profile の後に drain される（参照依存順）。
...
- `sonic-swss/cfgmgr/buffermgrdyn.cpp` L42-L51, L447-L456, L3536-3547, L3571
- `sonic-swss/orchagent/bufferorch.cpp` L41, L53-L81, L2040-L2073
