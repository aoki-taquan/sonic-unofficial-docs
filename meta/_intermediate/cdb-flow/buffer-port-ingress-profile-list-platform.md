# BUFFER_PORT_INGRESS_PROFILE_LIST — Phase H: プラットフォーム差分
...
| DEL 操作 | APPL_DB から削除 | `// Not supported on Mellanox platform for now.` 注記付きで削除処理 (`buffermgrdyn.cpp:3443`) |

evidence:
- `buffermgr.cpp:476-480` — dynamic_buffer_model 検出してスキップ
- `buffermgrdyn.cpp:3387-3450` — `handleSingleBufferPortProfileListEntry()` SET/DEL 完全実装（ingress/egress 共通関数）
...
#### 注記: BUFFER_PORT_INGRESS_PROFILE_LIST への直接影響

`8_lane` / モデル番号ロジックは **PG バッファプロファイル名生成** に関わるもので、BUFFER_PORT_INGRESS_PROFILE_LIST キー・フィールドそのものには影響しない。ただし、このテーブルが参照する `BUFFER_PROFILE` の名前がプラットフォームによって異なる名称になることに注意。

DEL 動作の Mellanox 注記は **ingress / egress 両 profile list** の DEL パスに存在する（同一関数 `handleSingleBufferPortProfileListEntry` を共用）。削除処理自体は実行されるが、Mellanox での正式サポートはコメント上「現時点では未サポート」。

### trim 禁止制約 (orchagent)

`bufferorch.cpp` の `processIngressBufferProfileList()` では `isTrimmingEligible` チェックが実装されており (`bufferorch.cpp:1725-1731`)、`packet_discard_action=trim` のプロファイルは ingress リストに設定不可。この制約は SAI 仕様上の制限でベンダー共通に適用される。
...
BUFFER_PORT_INGRESS_PROFILE_LIST テーブル自体の SET/DEL 処理コードは vendor 条件分岐なしで動作する（非 Mellanox 向け DEL パスにも上記コメントが残っている点に留意）。
...
`bufferorch.cpp` を `voq` キーワードで全行検索した結果、`processIngressBufferProfileList()` / `processIngressBufferProfileListBulk()` / `processIngressBufferProfileListPost()` の各関数内に **`gMySwitchType == "voq"` 分岐は存在しない**。
...
### BUFFER_PORT_INGRESS_PROFILE_LIST への影響

| 項目 | 標準スイッチ | VOQ Chassis |
|------|------------|------------|
| `processIngressBufferProfileList` 実行 | 通常通り | **同一コードパスが実行される**（分岐なし） |
| 処理開始条件 | `isConfigDone()` | `isInitDone()`（より早い段階） |
| SAI 属性 | `SAI_PORT_ATTR_QOS_INGRESS_BUFFER_PROFILE_LIST` | 同じ属性（VOQ 固有の代替なし） |
| BufferMgrDynamic (`buffermgrdyn.cpp`) | voq 分岐なし | voq 分岐なし（同一コードパス） |

**結論**: VOQ Chassis において BUFFER_PORT_INGRESS_PROFILE_LIST の挙動は標準スイッチと同一。voq 固有のキー拡張（`tokens.size() == 4` 形式）は BUFFER_QUEUE にのみ適用される。ingress profile list では `isInitDone()` による開始タイミングの早期化のみが間接的に影響する。

evidence:
- `bufferorch.cpp:2079-2094` — `doTask(Consumer)` 内の voq / 非 voq 分岐 (`isInitDone` vs `isConfigDone`)
- `bufferorch.cpp:1660-1774` — `processIngressBufferProfileList`: voq 条件分岐なし
- `bufferorch.cpp:1776-1965` — `processIngressBufferProfileListBulk` / Post: voq 条件分岐なし
...
| trim プロファイル禁止 | **あり** | ingress 専用制約（egress には同等制約なし） |
