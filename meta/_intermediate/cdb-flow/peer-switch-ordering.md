...
スキャン範囲: `handlePeerSwitch()`、`handleMuxCfg()`、`createStandaloneTunnelRoute()`、`processCachedNeighUpdates()`
...
### 4. Neighbor キャッシュ更新との関係

- `MuxOrch::processCachedNeighUpdates()` (muxorch.cpp:2483-2488):
  ```cpp
  if (mux_peer_switch_.isZero())
  {
  ...
  ```
  PEER_SWITCH 未確定時は neighbor 更新が全スキップ。PEER_SWITCH 確定後に `cached_neigh_updates_`
  が一括処理される。

### 5. PEER_SWITCH DELETE 未実装
...
| 4 | PEER_SWITCH 確定 → cached neighbor 更新処理 | 必須（未確定時は全スキップ） | PEER_SWITCH 確定後に自動処理 |
| 5 | PEER_SWITCH DEL → orchagent が旧 IP 保持 | 未実装（実質不可逆） | orchagent 再起動 |
| 6 | PEER_SWITCH 変更（DEL + SET） | YANG max-elements 1 + DEL 未実装のため orchagent 再起動必須 | — |
